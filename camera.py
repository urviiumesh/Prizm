...
def detect_document_contour(frame):
    ...
    for contour in contours:
        ...
        if len(approx) == 4:
            ...
            if 0.1 < aspect_ratio < 3:  # Filter aspect ratio
                return approx
    return None
...
def chatbot():
    ...
    full_text = " ".join(scanning_state["captured_texts"]) + " " + user_input +"answer to the point and do not repeat the statement"
...
def summarize_text():
    ...
    cleaned_text = block_personal_info(full_text) + "Summarize this text breaking every point into simpler and easy to understand terms ,also describe how each clause in the affects me since i have visual disability"
...
def upload_image():
    try:
        ...
        last_extracted_text = " ".join(extracted_text)

        # Generate summary of only the last extracted text
        summary = summarize_with_gemini_api(last_extracted_text)

        # Update the last extracted text and summary
        scanning_state["captured_texts"].append(last_extracted_text)
        scanning_state["last_summary"] = summary

        return jsonify({
            "extracted_text": last_extracted_text,
            "summary": summary
        })
